...
const getTradeTypeRates = async (req, res) => {
  ...
  try {
    ...
    const binds = {
      ...
      fromDate: { val: fromDate, type: oracledb.STRING }, 
    ...
    console.error("Error fetching trade type rates:", error);
    if (!res.headersSent) {
      const clientError = {
      ...
      res.status(500).json({
        success: false,
        message: "Internal Server Error during fetching trade type rates.",
        error: clientError,
      });
    }
  }
...
module.exports = getTradeTypeRates;
