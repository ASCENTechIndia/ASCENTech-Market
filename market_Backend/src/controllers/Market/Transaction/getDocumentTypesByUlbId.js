...
const getDocumentTypesByUlbId = async (req, res) => {
  let connection;
  let result;
  ...
  try {
    ...
    console.error("Error fetching document types by ULB ID:", error);
    if (!res.headersSent) {
      const clientError = {
      ...
      res.status(500).json({
        success: false,
        message: "Internal Server Error during fetching document types by ULB ID.",
        error: clientError,
      });
    }
  } 
...
module.exports = getDocumentTypesByUlbId;
