...
/**
 * API endpoint to retrieve a detailed list of receipts based on various filters.
 ...
 * }
 */
const getFilteredReceiptDetails = async (req, res) => {
  let connection;
  let result;
  ...
  try {
    ...
    // Trim and validate date strings - using DD-MM-YYYY format
    ...
    const sqlQuery = `
      SELECT
        a.dat_recipt_insdate AS RecieptInsDate,
        a.var_recipt_rcptno AS RecieptNumber,
        apm.var_appli_placeownername AS Name,
        NVL(apm.num_appli_amount, 0) AS Amount
      FROM
        MARKET.aomk_recipt_def a
      INNER JOIN
        MARKET.aomk_appli_mas apm ON a.num_recipt_appliid = apm.num_appli_id
        AND a.num_recipt_applino = apm.var_appli_applino
        AND apm.num_appli_wardid = a.num_recipt_collcenterid
      INNER JOIN
      ...
      WHERE
        TRUNC(a.dat_recipt_insdate) BETWEEN TO_DATE(:FromDt, 'DD-MM-YYYY') AND TO_DATE(:ToDt, 'DD-MM-YYYY')
    ...
    console.error("Error fetching filtered receipt details:", error);
    if (connection) {
      try {
    ...
    if (!res.headersSent) {
      ...
      if (clientError.errorNum) { // Changed from error.errorNum to clientError.errorNum
        clientError.oracleErrorNum = clientError.errorNum;
      }
      res.status(500).json({
        success: false,
        message: "Internal Server Error during fetching filtered receipt details.",
        error: clientError,
      });
    }
  } 
...
module.exports = getFilteredReceiptDetails;
