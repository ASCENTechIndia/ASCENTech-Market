...
const getChallanReport = async (req, res) => {
  let connection;
  let result;
  ...
  try {
    ...
    if (!ulbId || !FromDt || !ToDt) {
      console.error("Validation Error: Missing required parameters in request body (ulbId, FromDt, ToDt).");
      return res.status(400).json({
        success: false,
        message: "Missing required parameters: ulbId, FromDt, and ToDt are mandatory.",
      });
    }
    ...
    if (isNaN(parsedUlbId)) {
      console.error(`Validation Error: Invalid input for ulbId: ${ulbId}. Expected a number.`);
      return res.status(400).json({
        success: false,
    ...
    if (ZoneId !== undefined && ZoneId !== null && String(ZoneId) !== '-1' && String(ZoneId).trim() !== '') {
      parsedZoneId = parseInt(ZoneId, 10);
      if (isNaN(parsedZoneId)) {
        console.error(`Validation Error: Invalid input for ZoneId: ${ZoneId}. Expected a number or '-1'.`);
        return res.status(400).json({
          success: false,
    ...
    if (PayModeId !== undefined && PayModeId !== null && String(PayModeId) !== '-1' && String(PayModeId).trim() !== '') {
      parsedPayModeId = parseInt(PayModeId, 10);
      if (isNaN(parsedPayModeId)) {
        console.error(`Validation Error: Invalid input for PayModeId: ${PayModeId}. Expected a number or '-1'.`);
        return res.status(400).json({
          success: false,
    ...
    const sqlQuery = `
      SELECT *
      FROM view_ChallanReport
      WHERE ulbid = :ulbId
      AND TRUNC(trans_date) BETWEEN TO_DATE(:FromDt, 'DD-MM-YYYY') AND TO_DATE(:ToDt, 'DD-MM-YYYY')
      AND (-1 = :ZoneId OR wardid = :ZoneId)
      AND (-1 = :PayModeId OR paymodeid = :PayModeId)
      ORDER BY trans_date DESC -- Added an ORDER BY for consistent results, often helpful for reports
    `;
    ...
    if (!res.headersSent) {
      ...
      res.status(500).json({
        success: false,
        message: "Internal Server Error during fetching Challan Report.",
        error: clientError,
      });
    }
  }
...
module.exports = getChallanReport;
