...
const FrmPrintMarketReports = async (req, res) => {
  let connection;
  let result;
  ...
  try {
    ...
    if (isNaN(parsedTradeCategoryId)) {
      console.error(`Validation Error: Invalid tradeCategoryId: ${tradeCategoryId}. Expected a number.`);
      return res.status(400).json({
        success: false,
        message: `Invalid format for tradeCategoryId: ${tradeCategoryId}. Expected a number.`,
      });
    }
    ...
    const sqlQuery = `
      ...
      INNER JOIN
        ...
        AND arm.num_rate_ulbid = atm.aomk_tradetype_ulbid -- Assuming aomk_tradetype_ulbid is the correct column name
      WHERE
        atm.var_tradetype_flag = 'Y'
        AND atm.aomk_tradetype_tradecategoryid = :tradeCategoryId
        AND arm.num_rate_ulbid = :ulbId
      ORDER BY
        arm.num_rate_tradetypename -- Order by trade type name for consistent results
    `;
    ...
    console.error("Error fetching trade types by category and ULB:", error);
    if (!res.headersSent) {
      const clientError = {
...
module.exports = {FrmPrintMarketReports,FrmPrintSankshil, FrmPrintTapshil};
